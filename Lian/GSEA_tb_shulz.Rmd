...
gsea_MgSa_df <- gsea_MgSa_df[which(gsea_MgSa_df$metric != -Inf),] #removing metric   where = -Inf
gsea_MgSa_df <- gsea_MgSa_df[which(gsea_MgSa_df$metric != Inf),]

gsea_MgPv_df <- gsea_MgPv_df[which(gsea_MgPv_df$metric != -Inf),] 
gsea_MgPv_df <- gsea_MgPv_df[which(gsea_MgPv_df$metric != Inf),] 

gsea_PvSa_df <- gsea_PvSa_df[which(gsea_PvSa_df$metric != Inf),]
gsea_PvSa_df <- gsea_PvSa_df[which(gsea_PvSa_df$metric != -Inf),] 
#creating stats and pathways for gsea
  #MgSa
...
bplapply(1:10, sqrt, BPPARAM = SnowParam(fgsea(as.list(clust1_bio), ranks_MgPv, minSize = 1,  nperm = 100)))
?bplapply
for (i in 1:9){
...
fgsea(as.list(all_fuzzy$cell1), ranks_MgPv, minSize = 1,  nperm = 100)
