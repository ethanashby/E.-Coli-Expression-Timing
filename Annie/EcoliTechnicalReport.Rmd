...
```{r run DESeq2 for normalization, results='hide', message=FALSE, warning=FALSE}
...
According to **CITATION**, DESeq2 determines differential expression by:  
...
**(4)** comparing normalized counts with a negative binormial distribution with the sample mean, $\mu_j$, and gene-wise dispersion parameter, $\alpha_j$ to determine the significance of differential expression.

Because we have already run DESeq() above, we can now extract the significant results. We chose a p-value of 0.01 to be consistent across all of our work.
...
Run ImpulseDE2 with parameters specifying the count matrix, design matrix, the type of analysis: case-control instead of case-only, 
```{r run ImpulseDE2, results='hide', message=FALSE, warning=FALSE}
...
maSigPro is another serial method that determines genes with significant expression differences over time and between conditions. According to Conesa and Nueda, by:
...
```{r run maSigPro, results='hide', message=FALSE, warning=FALSE}
# ID DEGs
...
We first made lists of genes from all of the combinations of intersections between each method and then took the union of that list:
...
# in case you would rather be working with a smaller selection of genes, you can calculate the intersection of all three methods' gene lists here:
...
EC_counts_gath <- EC_DEGs %>%
  # gather count data for each sample by gene, so that each sample and replicate is its own line
...
```{r calc optimal k, message=FALSE, warning=FALSE}
...
EC_counts_sum %>%
  ...
  geom_line(aes(group = genename, color = as.factor(cluster)), lineend = "round" , alpha = 0.5) +
  facet_wrap(~factor(cluster, labels = c("Cluster 1", "Cluster 2", "Cluster 3"))) + geom_line(data = EC_counts_sum_med, aes(x=time, y=avecount, group = genename), size = 1) +
