...
for (i in 1:6) {
  ...
  resSig <- de_results[ which(de_results$padj < 0.0167 ), ]
...
library("mclust")
library(readr)
ec_Counts <- norm_counts_contrast %>%
  select(-contrast)
```
...
```{r}
sens <- read_csv("~/Downloads/sensitivitydata.csv") %>%
  select(c(geneName,bNum,`Direction of regulation`, sensitivity))

colnames(sens) <- c("genename","bnum","direction of regulation", "sensitivity")

sens_compare <- inner_join(joint, sens, by = c("bnum")) %>%
  filter(!is.na(bnum))
```
...
df_sens_prop <- df_sens %>%
  ...
  select(c(insensitive, sensitive)) %>%
...
df_sens_prop$contrast <- c("60","90","120","150")

df_sens_prop %>%
  gather(group, props, -contrast) %>%
  ggplot(aes(x = as.numeric(contrast), y = props, fill = group)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "time", y = "sensitivity proportion") +
  geom_line(size = 1, lineend = "round", aes(color = group)) +
  scale_fill_manual(values = c("#E69D2E", "#4087CE")) +
  scale_color_manual(values = c("#9E621E", "#1E4C9E"))
```
