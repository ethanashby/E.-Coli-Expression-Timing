...
for (i in 1:6) {
  ...
  resSig <- de_results[ which(de_results$padj < .0005 ), ]
  
  resSig <- resSig[ which(abs(resSig$log2FoldChange) > 3 ), ]
...
de_results <- results(ec_dds_de, contrast = c("timetreat", WT_list[idx],dRpoS_list[idx]))
  
resSig <- de_results[ which(de_results$padj < padj ), ]
  
resSig <- resSig[ which(abs(resSig$log2FoldChange) > fc ), ]
...
ec_counts_sum %>%
  ...
  geom_line(aes(color = gtreat, linetype = treat), alpha = 0.5) +
