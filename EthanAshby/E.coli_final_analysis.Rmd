...
#read in data
setwd('/Users/ethanashby/Desktop/E. Coli')
allCounts <- read.csv("/Users/ethanashby/Desktop/E. Coli/LB_Time_Course_GCA_000005845.2_ASM584v2_genomic_counts.tsv", header = T, sep = "\t")
...
set.seed(10)
genes<-c("talB", sample(monoDE, 9))
genes<-c("talB")
pdf("talB.pdf", onefile = TRUE)
...
repmeans<-c(rep(means, each=3))*size_facs
...
differentially_expressed<-matrix(ncol=18, nrow=0)
for (i in 1:100){
  expressions<-c()
  for (j in 1:18){
    expressions<-c(expressions, rnbinom(n=1, mu=repmeans[j], size=disp))
  }
  differentially_expressed<-rbind(differentially_expressed, expressions)
}
...
b<-ggplot(sim_params_to_plot, aes(x=beta, y=..density..))+geom_density()+geom_vline(xintercept=a$beta, color="red")+ggtitle(paste("Param Dist. of 1000 E coli genes simulated from", genes[k], "w/ disp.=", round(disp, 2), sep=" "))+labs(subtitle=paste("Params: beta", round(as.numeric(a[1]), digits=2), ",", names(a[2]), round(as.numeric(a[2]), digits=2), ",", names(a[3]), round(as.numeric(a[3]), digits=2), ",", names(a[4]), round(as.numeric(a[4]), digits=2)))+theme(plot.title = element_text(size = 7, face = "bold"), plot.subtitle = element_text(size = 5, face = "italic"))
c<-ggplot(sim_params_to_plot, aes(x=t, y=..density..))+geom_density()+geom_vline(xintercept=a$t, color="blue")+xlim(-50, 150)
d<-ggplot(sim_params_to_plot, aes(x=h0-h1, y=..density..))+geom_density()+geom_vline(xintercept=a$h0-a$h1, color="green")

#plot the initial trajectory on standard and log scales
#par(mfrow=c(1,2))
#plot(times[1:18], ecounts[match(genes[k], rownames(ecounts)), grep("JH01", colnames(ecounts))]*size_facs, main="Sigmoid fit of talB", xlab="time (mins)", ylab="counts")+lines(unique(times), means)
#plot(times[1:18], log2(ecounts[match(genes[k], rownames(ecounts)), grep("JH01", colnames(ecounts))]*size_facs), main=paste("Sigmoid Fit of talB"), xlab="time (mins)", ylab="log2(counts)")+ lines(unique(times[1:18]), log2(means))
...
counts_to_plot<-ecounts[match(genes[1], rownames(ecounts)),grep("JH01", colnames(ecounts))]*size_facs
counts_to_plot<-rbind(counts_to_plot, differentially_expressed*size_facs)
...
e<-ggplot(plot_trajectories[plot_trajectories$series!="init",], aes(x=time,y=log2(value))) + geom_line(aes(color=series), alpha=0.5, size=0.3)+ geom_line(data=plot_trajectories[plot_trajectories$series=="init",], aes(x=time, y=log2(value)),size=1, alpha=1)+theme_bw()+theme(legend.position ="none")+ggtitle(paste(genes[k], ": Plots of all expression trajectories (init in black)", sep=" ")) + geom_jitter(data=df, aes(x=time, y=log2(value), alpha=0.3), color="gray47",size=0.5)+scale_x_continuous("time", breaks=c(0,30,60,90,120,150), labels=c(0,30,60,90,120,150))+geom_vline(xintercept=sens_peak, linetype="dashed", color="cyan", size=0.5) +geom_vline(xintercept=insens_peak, linetype="dashed", color="magenta", size=0.5)+geom_vline(xintercept=monoparamsdf[genes[k],]$t, linetype="dashed", color="darkblue", size=0.5)
...
for (i in 1:length(sensvec)){
  index=match(sensvec[i], rownames(sigparamsdf))
  sensitivedf[i,]<-sigparamsdf[index,]}
...
for (i in 1:length(insensvec)){
  index=match(insensvec[i], rownames(sigparamsdf))
  insensitivedf[i,]<-sigparamsdf[index,]} 
colnames(insensitivedf)<-c("beta", "h0", "h1", "t")

#t params of montonic genes, looks somewhat bi/trimodal
ggplot(monoparamsdf, aes(x=t))+geom_density(color="blue", fill="cyan", binwidth=0.5)+ xlim(50,75)+ggtitle("Distribution of sigmoid t params of 740 monotonic E. coli DEGs")
...
for (i in 1:length(sensitive$geneName)){
  ...
  if (sensitive$geneName[i] %in% linear_genes){
    foo()
  }}
...
for (i in 1:length(insensitive$geneName)){
  ...
  if (insensitive$geneName[i] %in% linear_genes){
    foo()
  }}
...
#pull out 3 monotonic sensitive and 3 monotonic insensitive genes
set.seed(11)
genes<-c(as.vector(sensitive_genes[c(1,3,4)]), as.vector(insensitive_genes[c(5, 7, 8)]))
pdf("sens_insens_linear.pdf", onefile = TRUE)
for (k in 1:length(genes)){
  if (is.na(monoparamsdf[match(genes[k], rownames(monoparamsdf)),])==TRUE){foo()}
  else{
  geneparams<-monoparamsdf[match(genes[k], rownames(monoparamsdf)),]
