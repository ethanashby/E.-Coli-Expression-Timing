...
## Introduction
  ...
  ![(A) osmY: a linear gene (B) astA: a sensitive gene, (C) gadC: an insenstive gene. Figure obtained from Wong *et al.* 2017](./Steobel_figure_sens_insens_linear.png)
...
```{r Rpos kinetics, cache=TRUE, eval=T, echo=F, warning=FALSE, message=FALSE, results='hide', fig.keep='all'}
...
ggplot(meanRpos, aes(x=Time, y=Mean_RpoS))+geom_point()+geom_smooth()+ggtitle("RpoS kinetics over 150 min cell starvation time course")+theme_bw()
```
...
### Identifying Differentially Expressed Genes
  ...
  **DESeq2** (Love *et al* 2014) is an algorithm that models the read counts using a negative binomial distribution and uses a relative log expression (RLE) method to normalize the read count data across between-sample effects. DESeq2's main innovation is its Bayesian Shrunken Dispersion and LFC estimates, which improve stability and interpretability of these estimates (Love *et al* 2014). DESeq2 is one of the most widely accepted and empolyed algorithms for differential expression analysis. However, DESeq2 is not built to analyze serial (time course) data, and its categorical treatment of serial data leads to loss in power to identify DEGs. Thus, DESeq2 alone is an insufficient method to identify DEGs in TC experiments.
...
draw.triple.venn(area1= length(rownames(res)), area2 = length(maSigProGenes), area3= length(impulse_ecoli$vecDEGenes), n12=area1, n23=area3, n13=area2, n123=area123, category = c("DESeq2","maSigPro", "ImpulseDE2"), lty = rep("blank",3), fill = c("light blue", "light green", "pink"), cex=rep(3, 7), cat.cex=rep(2.25,3), cat.dist=c(0,0,0), cat.fontface = "bold")
```
...
###################################################################################################################
ecoli_DEGs<-union(intersect(impulse_ecoli$vecDEGenes, maSigProGenes), intersect(impulse_ecoli$vecDEGenes, rownames(res)))
```
...
ggplot(monoparamsdf, aes(x=t))+geom_density(color="blue", fill="cyan", binwidth=1)+xlim(50,100)+ggtitle("Distribution of sigmoid t params of 686 monotonic E. coli DEGs")+theme_bw()+theme(plot.title=element_text(size=18), axis.title = element_text(size=15), axis.text = element_text(size=12))
...
ggplot(transparamsdf, aes(x=t1))+geom_histogram(color="blue", fill="cyan", alpha=0.5, binwidth=0.5)+ xlim(0,100)+ggtitle("Distribution of 29 impulse t1 params of transient E. coli DEGs")
```
...
![Sigmoid function is descriptive of monotonic DEGs](./sigmoid_descriptive_1.png)
![Sigmoid function is descriptive of monotonic DEGs](./sigmoid_descriptive_2.png)
...
```{r Exploratory Plots and Wilcox, cache=TRUE, warning=FALSE, eval=TRUE, echo=FALSE, message=FALSE}
#####################
...
```{r, cache=TRUE, echo=FALSE, eval=FALSE, message=F, fig.keep='none'}
####################################
