...
## Introduction
  ...
  The impulse model sophisticates the sigmoid model by multiplying $f_{\theta}(x)$ by $\cdot s(x, t_2, h_2, -\beta)$.
...
### Identifying Differentially Expressed Genes
  ...
  **DESeq2** (Love *et al.* 2014) is an algorithm that models the read counts using a negative binomial distribution and uses a relative log expression (RLE) method to normalize the read count data across between-sample effects. DESeq2's main innovation is its Bayesian Shrunken Dispersion and LFC estimates, which improve stability and interpretability of these estimates and reproducibility of analyses (Love *et al.* 2014). DESeq2 is one of the most widely accepted and empolyed algorithms for differential expression analysis. However, DESeq2 is not built to analyze serial (time course) data, and its categorical treatment of serial data leads to loss in power to identify DEGs (Spies *et al. 2019*). Thus, DESeq2 alone is a suboptimal method to identify DEGs in TC experiments.
...
The distribution of t params for monotonic DEGs obtained from sigmoid models were plotted. Around 92% of the monotonic DEGs have onset times between 55 and 75 minutes, suggesting that the vast majority of monotonic DEGs turn on at around the same time.
...
#overlay plots for all transient DEGs
...
for (i in 1:length(other_genes)){
  ...
  tmpdf<-as.data.frame(cbind(times[1:18], t(WT_counts[index,])))
  rownames(tmpdf)<-NULL
...
The simulation pipeline was conducted under three conditions: 
1. With the *a priori* DESeq2-shrunk dispersion parameter estimate supplied to the rnbinom() function.
2. With a 100-fold *increase* in the dispersion parameter estimate (i.e. a 100-fold reduction in the noise caused by dispersion) supplied to rnbinom().
3. 100-fold *decrease* in the dispersion parameter estimate (i.e. a 100-fold increase in the noise caused by dispersion) supplied to rnbinom().

With unchanged dispersion, beta, t, h0, and h1 all appeared highly reproducible, as the initial parameter values (denoted by vertical red, blue, and green lines) were relatively central to the distributions of simulated values; computed quantiles of the initial parameter values (red, green, blue lines) in the distribution of simulated values are included on the plot (Figure 7). All expression trajectories were highly correlated (R^2>0.94) with the initial trajectory, and the simulated t-params showed low variability with an IQR of 2.11 minutes.The difference between the initial t param and median simulated t param was 0.64 mins, suggestive of stability of the t parameter.

```{r Normal Dispersion Sim, cache=T, eval=TRUE, echo=FALSE, message=FALSE, results='hide', fig.keep='all'}
##############################################################################################################
...
Figure 7: Above: distribution of simulated sigmoid parameters compared to 'true' sigmoid params. Below: plots of 'true' (black) and simulated (colored) sigmoid profiles and their simulated data points. Simulated profiles are colored based on correlation to 'true' profile. Initial/'true' t param (dotted blue line) and median simulated t param (dotted magenta line) are also plotted.

Increasing the dispersion-based noise lead to profound variability in the resulting simulated trajectories (Figure 8). The slope parameter (beta) was most affected by the additional noise. This distribution of simulated betas was not centered around the initial value. A substantial right skew suggestive of high variabilities in the simulated slope parameters. Amplitude parameters h0 and h1 were also affected by the dispersion-based noise, as the distribution of their difference (h1-h0) shifted considerably left from the initial value (over 30,000 units). 
...
```{r More noise, cache=T, eval=T, echo=FALSE, warning=FALSE, message=FALSE, results='hide', fig.keep='all'}
###########
...
Figure 8: Above: distribution of simulated sigmoid parameters compared to 'true' sigmoid params with dispersion noise increased 100-fold. Below: plots of 'true' (black) and simulated (colored) sigmoid profiles and their simulated data points when dispersion is increased 100-fold. Simulated profiles are colored based on correlation to 'true' profile. Initial/'true' t param (dotted blue line) and median simulated t param (dotted magenta line) are also plotted. t parameters remain relatively robust to the increase in noise, even when the simulated trajectories deviate from the 'true' trajectory.

```{r less noise, cache=T, eval=FALSE, echo=FALSE, warning=FALSE, message=FALSE, results='hide', fig.keep='all'}
##########
#Dispersion parameter decreased 100-fold
##########
...
One of the main advantages of the sigmoid model is that it is a parametric function, so expression levels can be attained at unknown time points. gadB's sigmoid model was used to calculate read counts at 15 time points (TPs): 0, 15, 30, 45, 55, 60, 65, 70, 75, 80, 85, 90, 120, 135, and 150 minutes, simulating data collection at additional time points. These time points were chosen because they predominantly focused at the time at which gadB expression spiked (between 60 and 90 mins). Simulated data at these 15 time points were generated with the DESeq2-estimated dispersion parameter supplied to 'rnbinom()' and sigmoid models were fit to these simulated data. As anticipated, the increase in number of time points afforded better resolution in the onset time (t) and slope (beta) parameters: the width of these distributions of simulated values shrunk and were both centered around the initial values. 
...
```{r More TPs, cache=T, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, results='hide', fig.keep='all'}
...
```{r Outlier Sim, cache=T, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, results='hide', fig.keep='all'}
###################
...
As shown below, the distributin of onset times for 87 sensitive and 15 insensitive monotonic DEGs overlap (Figure 13). Unexpectedly, a number of sensitive genes have onset times later than the large, single peak: 8 sensitive genes turned on later than 75 minutes (prpE, treA, ydcV, ydgD, astB, yeaG, crr, and cysQ).
...
```
Figure 13: Distribution of onset time params for sensitive and insensitive DEGs.
...
```{r Overlay Trajectories and Relative mRNA plot, cache=TRUE, warning=FALSE, eval=FALSE, echo=FALSE, message=FALSE, results='hide', fig.keep='none'}
#####################
...
```
Figure 14: Above: Relative mRNA plots of all sensitive and insensitive DEGs. Below: Relative mRNA plots of median sensitive and insensitive DEGs.
