...
ggplot(box_df, aes(x=time, y=value, fill=sens)) + geom_boxplot(position=position_dodge(1), outlier.size = 0, outlier.shape=NA, coef=0)+ylim(0,1)+theme_bw()+ylab("Relative mRNA level")+ggtitle("Expression Profiles of Sensitive and Insensitive Genes Derived From Sigmoid Imputation when Cells Enter Stationary Phase")+stat_summary(fun.y=median, geom="smooth", color="blue", aes(group=1))+stat_summary(data=insensdf, fun.y=median, geom="smooth", aes(group=2), color="maroon")
```
